...
const complianceStore = (function() {
  ...
  function calculateConflictList() {
    ...
    for (let i = 0; i < nodeCount; i++) {
      ...
      const currentLicense = resolveNodeLicense(nodeData, graph.getComplianceDetails(i));
      const baseModelTags = (nodeData.tags || []).filter(t => typeof t === 'string' && t.startsWith('base_model:'));
      ...
      baseModelTags.forEach(tag => {
        ...
        const parentLicense = resolveNodeLicense(parentData, graph.getComplianceDetails(parentNodeId));
